...
class Highlight_Search {
	...
	public static function init() {
		...
		add_action( 'wp', [ __CLASS__, 'add_url_filters' ] );
	...
	public static function enqueue_script() {
		...
		if ( ! self::have_search_terms() && ! self::have_hilite_terms() ) {
			return;
		}

		wp_enqueue_script( 'hlst-extend', plugins_url( 'hlst-extend' . ( defined( 'WP_DEBUG' ) && true == WP_DEBUG ? '' : '.min' ) . '.js', __FILE__ ), [], self::$version, true );
	...
	private static function try_search_terms() {
		// try know query vars.
		$query_vars = apply_filters( 'hlst_query_vars', [ 'search_terms', 'bbp_search' ] );
		...
		$input_get_args = apply_filters( 'hlst_input_get_args', [] );
